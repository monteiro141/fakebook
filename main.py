...
POSTS_USER = """
%s posts: #nome
(numero) %s %s %s """" # truth, posts e numero de coments"""
...
def addfriend_io(fakebook,args):
    ...
    second_userid = input()
    if not fakebook.has_user(first_userid) and not (fakebook.has_user(first_userid) and fakebook.has_user(second_userid)):
...
def friends_io(fakebook,args):
    ...
    if fakebook.has_user(userid):
        print(sorted(fakebook.friendsprint()))
        if fakebook.friendsprint() == []:
...
def post_io(fakebook,args):
    userid = args[0]
    sequence_of_hashtags = input()
    truthfulness = input()
    message = input()
    if not fakebook.has_user(userid):
        print(USER_NOT_EXISTS % (userid))
    else:
        fakebook.post(userid,sequence_of_hashtags,truthfulness,message)
        print(MESSAGE % (userid,truthfulness,len(fakebook.friendsprint())))
...
def comment_io(fakebook,args):
    ...
    if not fakebook.has_user(userid1) and not (fakebook.has_user(userid1) and fakebook.has_user(userid2)):
        print(USER_NOT_EXISTS % (userid1))
    elif not fakebook.has_user(userid2):
        print(USER_NOT_EXISTS % (userid2))
    else:
        print()
